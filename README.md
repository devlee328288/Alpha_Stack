...
| 대시보드가 **화면 안에서 학습** | Baseline CMA-ES · Model Lab 4모델 | 무료 CPU 에서 제한 · 재시작하면 캐시 소실 · 결과 캐시를 모든 접속자가 공유 | [#271](https://github.com/devlee328288/Alpha_Stack/issues/271) · [평가파트 v1.4](docs/평가파트/version1.4/대시보드_9페이지와_배포.md) |
| 대시보드 최고 모델 · 비용 단위 · 종목 라벨 | Model Lab 조합은 공식 C · K 로 맞췄다(#287 · #289) · 최고 모델은 조화평균 · 비용 프리셋은 왕복 이름인데 매수 · 매도에 각각 차감 · STOCK 라벨 ±1.0% — 논의 이슈 #264 · #265 · #267 · #268 은 09-15 닫힘(설정 공유만 #301 로 구현) | 화면 수치는 공식 결과와 다르다(LR 정확도 0.3736 vs 0.4542) · 보고서 숫자는 결과 파일에서 읽는다 | [#280](https://github.com/devlee328288/Alpha_Stack/issues/280) · [평가파트 v1.4 §4](docs/평가파트/version1.4/대시보드_9페이지와_배포.md) · [배포 화면 09-15](docs/media/배포화면/2026-09-15/README.md) |
| 백테스트 결과 (비용 차감 수익률 · Sharpe · MDD) | 결과 보고서 4.5절 빈자리 · 사전등록 주 검정 미수행 | 최종 결과가 분류 지표까지만 있다 · 공식 예측 파일은 봉인 폴더에 있다 | [결과 보고서 v1.2](docs/결과보고서/version1.2/) 9.2절 ① (연결 뒤 확인할 것) |
| `ruff` 220건 | `dashboard/` 206 · 그 밖 14(네 파일) | 형식만 · 기능 영향 없음 | [평가파트 v1.4 §5](docs/평가파트/version1.4/대시보드_9페이지와_배포.md) |
...
| **학습 최빈 기준선** (ADR 0006 · 개발구간 최빈 = 중립을 평가 전체에) | 0.1653 → **+26.78%p** | 0.2858 → **+9.49%p** |
| 참고 · 평가구간 최빈 (상승 · 정답을 본 사후값) | 0.5460 → −11.30%p | 0.3655 → +1.52%p |
...
- 기준선은 [ADR 0006](docs/decisions/0006-개별종목-중립대와-평가기준선.md) 정의대로 **학습에 쓴 개발구간의 최빈 클래스(중립)** 를 평가 전체에 예측한 정확도입니다. 두 트랙 모두 넘었습니다.
- 다만 중립 비율이 개발구간 38.6% · 38.0% 에서 평가구간 16.5% · 28.6% 로 줄어 차이가 부풀어 보입니다 — **MCC(KOSPI200 −0.0114 · 개별종목 0.0585)를 함께** 읽습니다. KOSPI200 은 0 에 가깝습니다.
- 평가구간 최빈(상승) 비율 0.5460 · 0.3655 는 정답을 보고 센 사후값이라 우위 판정에 쓰지 않습니다(ADR 0006 · 0007).
...
### 겪은 것 ③ — 기준선은 정답을 보기 전에 정한다

개발구간 기준선(0.3969)과 홀드아웃 정확도(0.3807)를 비교해 "기준선 아래로 내려왔다" 고 적었다가,
홀드아웃 구간의 최빈 비율(0.3655)로 다시 재 **+1.52%p 위**라고 고쳤다(PR #270). 그런데 그 비율은 평가구간 정답을 보고 센
사후값이었다(PR #278 닫힘 · ADR 0006). 기준선은 학습 구간에서 정한 규칙(최빈 = 중립)을 평가에 그대로 적용한다 —
기간을 맞추는 것과 정답을 보지 않는 것은 다른 조건이다.
...
| [발표 v1.1](docs/발표/README.md) | 🆕 [발표 최종통합본](docs/발표/version1.1/발표_최종통합본.html)(05~10 강민석) · v1.0 초안 · 데이터 파트 예상 질문 · 결과 보고서 초안(6.3 홀드아웃) |
...
| [평가파트 v1.4](docs/평가파트/version1.4/대시보드_9페이지와_배포.md) | 🆕 대시보드 9페이지와 배포 — main `cff6715` 로 다시 잼(조합 C · K · 설정 공유 · 전략 A~F) |
