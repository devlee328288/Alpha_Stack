...
def _close(days: pd.DatetimeIndex, seed: int = 0) -> pd.Series:
    rng = np.random.default_rng(seed)
    return pd.Series(100 * np.cumprod(1 + rng.normal(0, 0.01, len(days))), index=days)


def _ledger(days: pd.DatetimeIndex, close: pd.Series, *, probs=(0.5, 0.3, 0.2),
            kind: str = "signal") -> pd.DataFrame:
    """`run_backtest()` 가 내는 모양을 그대로 흉내 낸다 — 마지막 거래일은 행이 없다."""
    pred = days[:-1]
    exe = days[1:]
    rr = bl.recompute_realized_return(pred, close)
    base = {
...
def 개발구간():
    ...
    close = _close(days)
    return days, close, _ledger(days, close)
...
def test_규격에_맞는_원장은_문제가_없다(개발구간):
    days, close, df = 개발구간
    r = bl.verify_execution_log(df, holdout_start=HOLDOUT, close=close)
    assert r["problems"] == [], r
    assert r["rows"] == len(days) - 1
    assert r["summary"]["holdout_rows"] == 0
    assert r["summary"]["holdout_peek_rows"] == 0
    assert r["summary"]["realized_nan_rows"] == 4        # t+5 가 축 밖인 마지막 네 행
    assert r["summary"]["missing_columns"] == []


def test_종가를_안_주면_그_검사만_건너뛴다고_말한다(개발구간):
    _, _, df = 개발구간
...
def test_되먹임_칸이_하나라도_빠지면_잡는다(개발구간, 칸):
    _, close, df = 개발구간
    r = bl.verify_execution_log(df.drop(columns=[칸]), holdout_start=HOLDOUT, close=close)
...
def test_확률_합이_1이_아니면_잡는다(개발구간):
    ...
    r = bl.verify_execution_log(bad, holdout_start=HOLDOUT, close=close)
...
def test_손으로_적은_0_33_0_34_0_33_은_통과한다(개발구간):
    days, close, _ = 개발구간
    df = _ledger(days, close, probs=(0.33, 0.34, 0.33))   # 지금 predict_5d_after 의 값
    r = bl.verify_execution_log(df, holdout_start=HOLDOUT, close=close)
    assert r["problems"] == []
...
def test_실현수익률이_홀드아웃_종가를_엿보면_잡는다():
    # 예측일은 전부 개발구간(≤ 08-30)인데 t+5 종가가 09-02 이후인 행이 생기게 자료를 만든다
    days = _days("2024-08-05", 20)                      # 08-05 ~ 08-30
    close = _close(days)
    extra = pd.bdate_range("2024-09-02", periods=5)      # 봉인 구간 종가를 축에 붙인다
    close_full = pd.concat([close, _close(extra, seed=1)])
    df = _ledger(days, close)
    # 뒤 네 행의 실현수익률을 "봉인 구간 종가로 계산한 값" 으로 채운다 — 엿본 원장
    peeked = bl.recompute_realized_return(df["prediction_date"], close_full)
    df["realized_return_5d"] = peeked.to_numpy()
    assert df["realized_return_5d"].notna().all()
    r = bl.verify_execution_log(df, holdout_start=HOLDOUT, close=close_full)
...
def test_실현수익률이_종가와_다르면_잡는다(개발구간):
    _, close, df = 개발구간
    bad = df.copy()
...
def test_체결_원장에_hold_가_섞이면_잡는다(개발구간):
    days, close, _ = 개발구간
    df = _ledger(days, close, kind="trade")
    ok = bl.verify_trade_log(df, holdout_start=HOLDOUT, close=close)
    assert ok["problems"] == []
    df.loc[df.index[0], "action"] = "hold"
    r = bl.verify_trade_log(df, holdout_start=HOLDOUT, close=close)
...
def _market(days: pd.DatetimeIndex, close: pd.Series) -> pd.DataFrame:
    return pd.DataFrame({"open": close.shift(1).fillna(close.iloc[0]), "close": close}, index=days)
...
def 종단_원장(백테스트):
    ...
    close = _close(days)
    res = 백테스트.run_backtest(_market(days, close), days[0], days[-1], _predict_with_probs,
                            strategy="A", model_id="contract-v0", run_id="run_contract")
    return days, close, res
...
def test_종단_원장이_검증기를_통과한다(종단_원장):
    _, close, res = 종단_원장
    r = bl.verify_execution_log(res["signal_log"], holdout_start=HOLDOUT, close=close)
    assert r["problems"] == [], r
    t = bl.verify_trade_log(res["trade_log"], holdout_start=HOLDOUT, close=close)
    assert t["problems"] == [], t
...
def test_종단_실현수익률은_종가_5거래일_수익률이다(종단_원장):
    _, close, res = 종단_원장
    s = res["signal_log"]
    mine = bl.recompute_realized_return(s["prediction_date"], close)
    both = s["realized_return_5d"].notna().to_numpy() & mine.notna().to_numpy()
    assert both.sum() == len(s) - 4
...
def test_종단_구버전_예측함수도_기본_확률로_채워_통과한다(백테스트):
    ...
    close = _close(days, seed=2)
    res = 백테스트.run_backtest(_market(days, close), days[0], days[-1], _predict_legacy,
                            strategy="B")
    s = res["signal_log"]
    assert (s["signal"] == "상승").all()
    assert np.allclose(s[["p_up", "p_flat", "p_down"]].sum(axis=1), 1.0, atol=bl.PROB_TOL)
    r = bl.verify_execution_log(s, holdout_start=HOLDOUT, close=close)
    assert r["problems"] == [], r
...
def test_종단_홀드아웃을_넘겨_돌리면_검증기가_막는다(백테스트):
    ...
    close = _close(days, seed=3)
    res = 백테스트.run_backtest(_market(days, close), days[0], days[-1], _predict_with_probs,
                            strategy="A")
    r = bl.verify_execution_log(res["signal_log"], holdout_start=HOLDOUT, close=close)
    assert r["summary"]["holdout_rows"] > 0
    assert r["summary"]["holdout_peek_rows"] > 0        # 08-26 예측의 t+5 가 09-02
