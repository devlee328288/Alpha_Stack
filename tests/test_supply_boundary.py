...
def test_아직_몰랐던_거래일은_나오지_않는다(채워진저장소):
    ...
    rows = supply.index_series(as_of="2026-08-25")

    assert [r["date"] for r in rows] == ["2026-08-21", "2026-08-24"]
    assert "2026-08-25" not in [r["date"] for r in rows], "당일 자료가 새어 나왔다"
...
def test_as_of_를_하루_늦추면_하루가_더_보인다(채워진저장소):
    ...
    assert 오늘[-1]["date"] == "2026-08-25"


def test_부르는_쪽이_준_end_보다_as_of_가_이기지_못한다(채워진저장소):
    """end 를 넉넉히 줘도 as_of 를 넘지 못한다 — 둘 중 이른 쪽이 이긴다."""
    rows = supply.index_series(as_of="2026-08-25", end="20991231")

    assert [r["date"] for r in rows] == ["2026-08-21", "2026-08-24"]


def test_end_가_as_of_보다_이르면_end_가_이긴다(채워진저장소):
    rows = supply.index_series(as_of="2026-08-26", end="20260821")
...
def test_언제부터_알_수_있었는지를_행에_붙여_준다(채워진저장소):
    ...
    rows = supply.index_series(as_of="2026-08-26", with_known_at=True)

    for row in rows:
        assert row["known_at"] > row["date"], "알게 된 시각이 거래일보다 앞설 수 없다"
    assert rows[-1]["known_at"].startswith("2026-08-26T00:00:00")
