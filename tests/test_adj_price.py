...
# ==================================================
# 🔴 관문 ③ — FDR 이 이미 KRX 기준가만큼 편 자리 (2026-09-05)
# ==================================================

#: 삼양제넥스(003940) 2013-03-25 인적분할을 원문 그대로 옮긴 표본.
#: 🔴 앞 사흘이 거래정지라 계수가 **상장주식수 배율**에서 나온다 — 그런데 인적분할은
#:    주식수가 줄어도 가격이 그대로여서 그 계수가 틀린다.
인적분할표본 = [
    _row("20130321", 0, 0, 0, 68900, 0, 2985917, volume=0),
    _row("20130322", 0, 0, 0, 68900, 0, 2985917, volume=0),
    _row("20130325", 78000, 81000, 70500, 78000, 7500, 1905907, volume=79073),
    _row("20130326", 78600, 79800, 76100, 78000, 0, 1905907, volume=17546),
]

#: 그 날 FDR 이 실제로 준 값. 🔴 정지일에 원종가(68,900)가 아니라 **KRX 기준가
#: (70,500)** 를 준다 — FDR 이 이 사건을 알고 이미 폈다는 증거다.
인적분할FDR = {"20130321": 70500.0, "20130322": 70500.0,
              "20130325": 78000.0, "20130326": 78000.0}


def test_FDR_이_KRX_기준가만큼_이미_폈으면_건드리지_않는다():
    """🔴 관문 ③. 관문 ①②만으로는 못 막아 실제로 한 번 통과했다.

    주식수가 0.638배로 줄어 계수가 `1.5667` 로 나오지만, KRX 기준가는 70,500 —
    직전 종가 68,900 의 **1.0232배**밖에 안 된다. 인적분할이라 가격이 안 끊긴 것이다.
    FDR 이 편 폭도 정확히 1.0232 다.

    관문 ①은 `|1.1321/1.5667 - 1| = 0.2774` 로 임계 0.30 안쪽이라 통과하고,
    관문 ②도 `|1.0232 - 1| = 0.0232` 로 임계 0.10 안쪽이라 통과한다.
    막는 것은 **FDR 이 편 폭이 KRX 기준가가 움직인 폭과 같다**는 사실뿐이다.

    이 관문이 없으면 그 종목의 **과거 803행이 1.567배로 부풀어** 오른다.
    """
    scales = adj_price.scale_series(인적분할표본, 인적분할FDR)
    # FDR 이 심은 값 그대로여야 한다 — ⑤가 손댔다면 여기에 1.5311 이 곱해진다.
    assert scales[0] == Fraction(70500) / Fraction(68900)
    assert scales[1] == Fraction(70500) / Fraction(68900)
    assert scales[2] == Fraction(1)


def test_관문3이_막은_뒤_그날_수익률이_KRX_등락률과_일치한다():
    """편 결과가 옳은지는 **바깥 값**으로 잰다 — 여기서는 소수점까지 맞는다.

    FDR 이 정지일에 기준가(70,500)를 줬으므로 `78,000/70,500 - 1 = +10.64%` 이고
    KRX 등락률(`7,500 / 70,500`)과 **정확히 같다.** 우리가 폈다면 `-27.74%` 였다.
    """
    by_day = {r[-1]: r for r in adj_price.build_rows(인적분할표본, {
        d: {"adj_close": v} for d, v in 인적분할FDR.items()})}
    변화 = (by_day["20130325"][3] / by_day["20130322"][3] - 1) * 100
    krx = 7500 / 70500 * 100
    assert abs(변화 - krx) < 0.01
    assert abs(변화 - 10.64) < 0.01
    # 안 막았다면 이렇게 읽힌다 — 부호까지 뒤집힌다.
    assert abs((1.1321 / float(Fraction(2985917, 1905907)) - 1) * 100 + 27.74) < 0.1


def test_관문3은_KRX_가_기준가를_안_바꾼_날에는_발동하지_않는다():
    """`기준가비 == 1` 이면 대조할 것이 없다 — 그 날은 ①②로만 판정한다.

    감자인데 KRX 등락률이 `0.00%` 인 자리가 실재한다(참엔지니어링 2026-05-11).
    여기서 ③이 발동하면 **진짜 감자를 놓친다.**
    """
    표본 = [
        _row("20260508", 0, 0, 0, 1000, 0, 5000000, volume=0),
        _row("20260511", 5000, 5000, 5000, 5000, 0, 1000000),   # 5:1 감자 · 등락 0
    ]
    fdr = {"20260508": 1000.0, "20260511": 5000.0}   # FDR 이 안 폈다
    scales = adj_price.scale_series(표본, fdr)
    assert scales[0] == Fraction(5)      # 폈다
    assert scales[1] == Fraction(1)
