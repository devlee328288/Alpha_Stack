...
행 수를 세는 검사로는 ①·③이 안 잡힌다. 그래서 여기서 못박는다.
"""
...
@pytest.fixture
def 기본정보DB(tmp_path):
    """`stock_base_info` 만 있는 작은 DB. `known_at` 은 basDd 의 다음 거래일이다."""
    경로 = tmp_path / "base.db"
...
def test_주권종류_세칸이_그대로_실린다(기본정보DB):
    frame = _시세(("20200102", "006800"), ("20200102", "000327"),
                  ("20200102", "088260"), ("20200102", "999999"))
...
def test_판정을_대신_하지_않고_원문을_싣는다(기본정보DB):
    ...
    """
    out = attach_security_type(_시세(("20200102", "088260"), ("20200102", "999999")),
...
def test_행_수와_순서가_보존된다(기본정보DB):
    ...
    frame = _시세(("20200102", "999999"), ("20200102", "006800"),
                  ("20200102", "088260"))
...
def test_기본정보가_없는_행은_비워_두고_터지지_않는다(기본정보DB):
    ...
    out = attach_security_type(_시세(("20200102", "000001")),
                               as_of="2026-09-09", db_path=str(기본정보DB))
...
def test_as_of_뒤에_알게_된_행은_안_붙는다(기본정보DB):
    ...
    out = attach_security_type(_시세(("20260901", "005930")),
                               as_of="2026-08-01", db_path=str(기본정보DB))
    assert out["kind_stkcert_tp_nm"].isna().all()

    늦게 = attach_security_type(_시세(("20260901", "005930")),
                                as_of="2026-09-09", db_path=str(기본정보DB))
...
def test_as_of_는_기본값이_없어_빠뜨릴_수_없다(기본정보DB):
    with pytest.raises(TypeError):
        attach_security_type(_시세(("20200102", "006800")))  # noqa: PGH001
