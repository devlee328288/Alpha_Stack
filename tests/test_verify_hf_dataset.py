...
⑤ 는 ④ 를 너무 넓게 적용했던 자리다(2026-09-11). 예전에는 문자열 칸의 차이가 결측
엇갈림뿐이면 전부 ④ 로 봤고, 상장 첫날 주권종류가 '보통주' → 결측으로 바뀐 3,462행이
⚪ 로 적혔다. CSV 는 '보통주' 를 지우지 못한다 — 그래서 **남은 쪽 값이 빈 문자열일 때만**
④ 다.
"""
...
def test_결측이_빈_문자열이_된_것도_표기의_한계다():
    """방향은 상관없다 — 왼쪽이 결측이고 오른쪽이 빈 문자열이어도 같은 표기 차이다."""
    d = V.compare_column(pd.Series([None, "우량기업부"]),
                         pd.Series(["", "우량기업부"]))
    assert d["행"] == 1
    assert d["표기한계"] is True


def test_문자열_값이_결측이_된_것은_변화다():
    """2026-09-11 재배포 판정에서 상장 첫날 주권종류가 '보통주' → 결측이 됐다.

    CSV 는 빈 칸을 결측으로 읽을 뿐 '보통주' 를 지우지 않는다. 그런데 예전 규칙(결측
    엇갈림뿐이면 표기 한계)은 이 3,462행을 ⚪ 로 적었다. 업종 칸이 함께 붉어서 판정이
    맞았을 뿐, 주권종류만 바뀌었다면 "재배포 불필요" 가 나왔을 자리다.
    """
    d = V.compare_column(pd.Series(["보통주", "보통주"]),
                         pd.Series([None, "보통주"]))
    assert d["결측엇갈림"] == 1
    assert d["표기한계"] is False


def test_빈_칸과_사라진_값이_섞이면_변화다():
    """빈 칸 하나는 표기지만 값이 사라진 행이 하나라도 섞이면 그 칸은 변화다."""
    d = V.compare_column(pd.Series(["", "보통주"]), pd.Series([None, None]))
    assert d["결측엇갈림"] == 2
    assert d["표기한계"] is False
