...
업종분류 현황은 연 1회 손으로 받은 스냅샷이다. 날짜 `d` 의 업종은 *"`d` 까지 알게 된 가장 최근
...
def test_그때_아직_몰랐던_스냅샷은_보이지_않는다(db, tmp_path):
    ...
    # 다음 거래일 행부터 보인다
    새 = sector.industry_as_of("20200103", as_of="2020-01-04", db_path=path)
    assert list(새["industry"]) == ["새"]


def test_as_of_를_넉넉히_줘도_스냅샷_당일_행에는_그날_스냅샷이_안_붙는다(db, tmp_path):
    """🔴 2026-09-11 에 고친 자리. 예전에는 스냅샷 **날짜**로 잘라서, `as_of` 를 넉넉히 주는
    학습·반출 경로에서 당일 행에 그날 마감 뒤에 나온 표가 붙었다. 실측 스냅샷 당일
    40,857행 중 5,300행이 새 정보를 하루 먼저 봤다.

    점 조회와 붙이기를 둘 다 본다. 그리고 **예전 규칙(날짜로 자르기)** 이면 '새' 가 붙는다는
    대조를 이 자리에서 만들어, 시험이 실제로 두 규칙을 가르는지 확인한다.
    """
    conn, path = db
    달력(conn, "20150102", "20150105", "20200102", "20200103", "20200106")
    스냅샷(path, tmp_path, "20150102", {"005930": "옛"})
    ...
    점 = sector.industry_as_of("20200102", as_of="2026-09-03", db_path=path)
    assert list(점["industry"]) == ["옛"]

    붙인 = sector.attach_industry(
        pd.DataFrame({"bas_dd": ["20200102", "20200103"], "code": ["005930", "005930"]}),
        as_of="2026-09-03", db_path=path)
    assert list(붙인["industry"]) == ["옛", "새"]
    assert list(붙인["industry_known_at"]) == ["20150105", "20200103"]

    snaps = sector.snapshots(db_path=path)
    예전 = snaps[snaps["bas_dd"] <= "20200102"].sort_values("bas_dd").iloc[-1]
    assert 예전["sector_nm"] == "새", "대조: 날짜로 자르면 당일 스냅샷이 잡힌다"
    assert 예전["known_at"] > "20200102", "대조: 그런데 그 스냅샷은 그날 알 수 없었다"
...
# ==================================================
def test_행마다_그_날짜까지_알게_된_가장_최근_스냅샷이_붙고_순서는_그대로다(db, tmp_path):
    conn, path = db
    달력(conn, "20150102", "20150105", "20200102", "20200103", "20200106")
    스냅샷(path, tmp_path, "20150102", {"005930": "옛"})
    ...
    # 20150102 행은 그날 스냅샷을 아직 모른다(known_at 20150105) — 앞선 스냅샷이 없어 빈 칸이다
    assert list(out["industry"].fillna("-")) == ["새", "-", "-", "옛", "새"]
    assert list(out["industry_bas_dd"].fillna("-")) == [
        "20200102", "-", "-", "20150102", "20200102"]
...
@pytest.mark.parametrize("업종, 지수", [
    ("은행", "금융"), ("기타금융", "금융"),
    ("전기·전자", "전기전자"), ("기타제조", "제조"),
    # 🔴 보험·증권은 자기 지수가 있다 — 금융으로 올리면 보험 지수에 붙는 종목이 0 이 된다
    ("보험", "보험"), ("증권", "증권"),
...
def test_상위_묶음_지수는_대조표의_목적지에_들어_있다():
    """`기타제조 → 제조` 처럼 대조표가 가리키는 지수가 묶음이면, 고르는 쪽이 뺄 수 있어야 한다."""
    assert sector.UMBRELLA_INDICES == {"제조", "금융"}
    올라간_곳 = {sector.index_name_for("기타제조"), sector.index_name_for("은행")}
    assert 올라간_곳 <= sector.UMBRELLA_INDICES
