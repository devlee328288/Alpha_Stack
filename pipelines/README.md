...
> 🟢 **수집(`ingest`)** 2026-09-02 · 🟢 **갱신(`refresh`)** 2026-09-05. 학습·평가는 아직 계획입니다.
...
### 갱신 — 받아서 검사하고 필요하면 팀에 내보낸다

```bash
python -m pipelines.refresh             # 수집 → 게이트 → 판정 → (필요할 때만) 반출·업로드
python -m pipelines.refresh --with-adj  # 수정주가까지 다시 만든다 (약 13분)
python -m pipelines.refresh --dry-run   # 무엇을 할지만 본다
python -m pipelines.refresh --status    # 최근 실행 기록
python -m pipelines.refresh --json      # 화면이 읽을 계약을 마지막 줄에
```

실측 2026-09-05 (수정주가를 끈 기본 실행, 전체 **4분 37초**):
...
| 수정주가 | — | `skipped` (기본값) |
| 품질 게이트 | 106초 | 통과 |
| 재배포 판정 | 152초 | **최신 — 올릴 이유 없음** |
| 반출 | — | `skipped` (판정이 최신) |
| HF 업로드 | — | `skipped` (판정이 최신) |

반출만 따로 재면 **185초**입니다 (`--only export --force-export`).

🔴 **수정주가는 기본으로 끕니다.** "최신으로 맞춘다" 가 여기서는 반대로 작동합니다.
FinanceDataReader 는 종목마다 최근 3,000거래일만 수정주가를 주고 그 창이 날마다
앞으로 밀립니다. 밀려난 행은 원본에서 우리 계산값(`chain`, 오차 상한 0.39%)으로
넘어갑니다. 실측(2026-09-05, 창이 20140617):

| 창이 밀린 정도 | 출처가 바뀌는 행 | 반출본 대비 |
|---|---:|---:|
| 20거래일 (약 한 달) | 11,502 | 0.146% |
| 60거래일 (약 세 달) | 38,339 | 0.486% |
| 250거래일 (약 한 해) | 158,996 | 2.02% |

날마다 돌리면 날마다 조금씩 나빠지고, 그때마다 판정이 "재배포 필요"를 내어
팀원이 404MB 를 다시 받습니다. 다시 만들 때는 **조정 코드가 바뀐 날**입니다
(2026-09-05 의 감자 관문 ③처럼). 그날은 `--with-adj` 로 켭니다.

### 화면이 읽는 계약

```json
{"run_id": "20260905-163736", "status": "ok",
 "stages": {"ingest": {"status": "ok", "seconds": 18.4, "note": "..."}, "…": {}},
 "uploaded": false}
```

`status` 는 넷입니다. `ok` · `busy`(이미 돌고 있음) · `gate_failed`(자료가 못 쓸 상태 —
사람이 봐야 하고 다시 눌러도 같은 곳에서 막힘) · `error`(수집 실패 — 다시 눌러 볼 만함).

버튼은 두 번 눌립니다. `data/refresh.lock` 이 막고, 두 번째 호출은 기다리지 않고
곧바로 `busy` 와 **진행 중인 `run_id`** 를 돌려줍니다. 화면이 그 열쇠로 이어서
폴링할 수 있어야 하기 때문입니다.

HF 쓰기 토큰은 팀장 PC 에만 있습니다. 팀원 PC 에서 누르면 판정·반출·업로드가
`skipped` 로 남고 수집·게이트까지는 정상으로 끝납니다 — 실패로 칠하면 팀원 화면이
늘 붉어서 진짜 실패를 못 알아봅니다.

### 진행 상황은 어디에 남나

`ingest_run` · `ingest_run_stage` 에 **시작할 때부터** 남습니다.
끝나고 한꺼번에 쓰면 중간에 죽었을 때 아무 흔적도 없어서 *"애초에 안 돌았다"* 와
구별되지 않기 때문입니다. 팀 대시보드는 이 두 표를 폴링합니다.

두 파이프라인이 같은 표를 나눠 쓰므로 `ingest_run.args` 의 `pipeline` 값으로 가릅니다
(`"ingest"` · `"refresh"`). ⚠️ 2026-09-05 이전 행에는 그 열쇠가 없습니다 — 그때는
수집만 이 표를 썼습니다.

자세한 것: [기능명세 v1.3](../docs/기능명세/version1.3/거시수집과_수집파이프라인.md) ·
[고도화 설계 3절](../docs/데이터파트/version3.6/고도화_설계.md)
