...
| 조합 | 조합별 최우수 모델 | 변형 | 게이트 | Accuracy | 기준선 대비 | 승리 폴드 | 상승 PR-AUC | 상승 Precision | ΔSharpe 중앙값 |
|---|---|---|---|---:|---:|---:|---:|---:|---:|
...
현재 개발구간 순위 표시는 1위 `⭐조합C`·2위 `❤️조합B`·3위 `♡조합E`입니다. 조합 C의
피처는 `sma_gap_5_20`, `macd_hist_ratio`, `rsi_14`, `bb_position`, `hv_20`,
`vol_ratio_20`입니다. 이슈 #216에서 조합 C LogisticRegression을 개발구간 최종 선정
모델로 확정했습니다. 전체 결과는
