...
참고 — `features/stock_model_dataset.py`의 `STOCK_NEUTRAL_BAND`(개별 종목 ±2% 고정
임계값)가 지금 이것과 같은 자리(종목 등락을 상승/중립/하락으로 가르는 경계)를 다룬다.
나중에 개별 종목에 유동 임계값을 적용하기로 하면, 그 상수를 이 함수 호출로 바꾸는
지점이다.
"""
