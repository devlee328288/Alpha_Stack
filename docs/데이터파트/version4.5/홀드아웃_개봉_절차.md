...
```powershell
# ① 가짜 개봉 — 기록은 reports/dryrun/unseal.log
python scripts/unseal_holdout.py open --dry-run --requested-by 이동원 --reason "통합 드라이런"

# ② 입력 넷 — 모델 파트 드라이런 준비기가 index_dev · index_holdout · stock_dev · stock_holdout .parquet 을 쓴다
python scripts/prepare_final_holdout_dry_run.py --output-dir <폴더>

#    그 넷으로 가짜 승인
python scripts/unseal_holdout.py authorize --dry-run `
  --snapshot data/sealed/dryrun_<시작>_20240831 --run-id dryrun-<날짜> `
  --index-dev <경로> --index-holdout <경로> --stock-dev <경로> --stock-holdout <경로>

# ③ 확인
python scripts/unseal_holdout.py check --dry-run --result-dir <실행기 드라이런 출력 폴더>
```
