...
### 2.9 🆕 결과 — 만들었고, 신호가 없었다 (2026-09-04 · v3.4 정정 블록)

§2.1~2.8 은 설계 시점(09-03)의 기록이라 고치지 않고 여기 결과를 덧붙입니다. 전문은
[기능명세 v1.5 텍스트 신호](../../기능명세/version1.5/텍스트_신호.md) ·
[노트북 06/05](../../../notebooks/06-검토·발견/05.신호가-없다도-재현-가능해야-한다.ipynb).

| 설계 | 실제 | 왜 |
|---|---|---|
| §2.1 원료 5,000~21,000건 (추정) | **1,555,556행 · 고유 제목 18,600개** | 350종이 아니라 법인 2,651곳 전체 · 날짜로 받았다 |
| §2.2 A. 원격 제로샷 `xlm-roberta-large-xnli` | **로컬 CPU `snunlp/KR-FinBert-SC`** (금융 감성 파인튜닝 모델 · 인용으로 표기) | 원격은 건당 $0.01~0.03 이라 상한을 넘는다. 로컬은 0원 · 44건/초 · 7분 — 규칙 §5 개정 |
| §2.4 표에 `code`·`rcept_no`·`known_at` | **제목 단위 캐시** `(text_sha, model_id)` — 시점은 반출 때 | 같은 문장이 평균 84번 반복된다 |
| §2.4 마이그레이션 v11 | **v12** | v11 은 종목기본정보가 먼저 (선착순) |
| §2.5 HF 예산 D-83 | **폐기** | 로컬이라 과금이 없다 |
| §2.7 분포 게이트 15~45% | **미달** (8.3 · 73.4 · 18.3%) → 효과크기로 판정 | 공시는 원래 대부분 중립 · 그 게이트는 라벨용 |
| §2.7 카이제곱 p < 0.05 | p < 1e-300 인데 **Cramer's V = 0.026** | 126만 행에서 p 는 언제나 유의하다. 효과크기가 "작음"(0.1)의 1/4 |
| §2.2 "B 도 못 넘으면 '신호가 없다' 가 결론" | **그 결론이다** — B(임베딩+분류기)는 가지 않았다 | 12조합 전부 V < 0.04 고 긍정이 중립보다 못하다. 분류기를 학습해도 라벨과 무관한 입력에서 신호가 나오지 않는다 |

🔴 **제목을 그대로 피처로 쓰면 시점이 샙니다.** `사업보고서 (YYYY.12)` 15개 연도가 전부 중립인데
5일 상승 비율은 11.1~81.0% — 연월이 시장 국면을 가리킵니다. 뉴스·본문으로 갈 때도 같은 함정을
먼저 봅니다.
