...
| 버전 | 날짜 | 무엇이 담겼나 |
|---|---|---|
| **[version1.3](version1.3/)** | 2026-09-14 | **홀드아웃 개봉 뒤 실제 구성으로 다시 그림** — v1.2 상자를 하나씩 코드와 대조([변경사항](version1.3/변경사항.md)) · 점선(DART · 통계 · features · models · 산출 파일 · 화면)이 실선으로 · 🆕 `backtest/` · 홀드아웃 봉인(붉은 테) · HF 두 레포 · 버튼 갱신 8단계 · `supply` 칸 셋(원천 정문 · 상장 구간) · 🔴 화면이 학습을 직접 돈다 |
