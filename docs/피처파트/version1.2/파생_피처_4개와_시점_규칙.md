# 파생 피처 4개 — 무엇을 재고, 언제까지의 자료를 쓰나 (v1.2)
...
네 파일 **218 passed · 3 xfailed** (2026-09-09 재측정 · anaconda 3.12.13). v1.1 의 216 에서
둘이 는 것은 아래 §4.1 의 회귀 시험입니다.

### 🆕 4.1 `hv_regime` baseline 판정에 eps 를 둔 이유 (v1.2 · PR #170)

신장환 님이 [PR #170](https://github.com/devlee328288/Alpha_Stack/pull/170)(2026-09-08)에서
`baseline > 0` 을 `baseline > 1e-9` 로 바꾸셨습니다. **선제적 수정**이고, 지금 학습 결과는
바뀌지 않습니다.

무엇이 문제였나 — `_rolling_mean` 은 `nancumsum` 누적합의 차분으로 이동평균을 냅니다. 큰 수끼리
빼는 방식이라 **취소오차**가 쌓이는데, `hv_regime` 의 `regime_window` 는 **250** 으로 다른
호출부(14·20일)보다 창이 훨씬 커서 그 오차가 커집니다.

| | 실측 (신장환 님 · PR #170 본문) |
|---|---|
| 대조 범위 | KOSPI 개발구간 전수 **1,224종목 · 2,984,824행** |
| 최대 절대오차 | `9.481e-11` (pandas `rolling().mean()` 기준) |
| `baseline > 0` 판정이 갈리는 행 | **73개** — 전부 저유동성 소형주의 거의 무변동 구간 |
| **조합 H 실제 학습 패널**(164종목 · 175,914행) | **불일치 0건** |

임계 `1e-9` 는 관측된 최대 오차(`9.481e-11`)보다 **한 자리 위**입니다. 뜬 오차로 인한 판정
흔들림만 걸러내고 실제 baseline 은 통과시킵니다. 근거가 코드 주석에 그대로 남아 있습니다
([`features/volatility.py:247`](../../../features/volatility.py#L247)).

🔴 **이 예외는 `hv_regime` 하나뿐입니다.** 저장소의 다른 0-나눗셈 가드(`atr_ratio` 의
`close != 0`, `historical_volatility` 의 `px > 0`)는 전부 경계를 `0` 으로 둡니다 — 창이 작아
같은 문제가 안 생기기 때문입니다.
...
| 시험 | 무엇을 못박나 |
|---|---|
| `test_hv_regime_baseline_eps_경계` | 임계 근처에서 판정이 갈리는 자리 |
| `test_hv_regime_baseline_완전히_0이면_nan` | baseline 이 진짜 0 이면 값이 아니라 NaN |

**데이터 파트가 배운 것** — 같은 패턴이 저장소 곳곳에 있습니다(`indicators.py`·`volume.py`·
`timeseries/transform.py` 가 다 `nancumsum` 을 씁니다). 지금은 창이 작아 문제가 없지만,
**창이 큰 지표를 새로 만들 때 같은 자리를 봐야 합니다.**
