...
> v1.3 (2026-09-10): **여덟 단계** — `action`(기업행위 표)이 ③으로 들어왔다 (§8)
> 🆕 v1.4 (2026-09-11): **봉인 반출** — 개발본과 같은 함수로, 개봉 기록과 함께만 (§9 · 이슈 #240)
...
## 8. 🆕 여덟 단계 — 버튼 한 번으로 반출까지 (v1.3 · 2026-09-10)

정본: [`pipelines/refresh.py`](../../../pipelines/refresh.py) · 시험 `tests/test_pipelines_refresh.py` 28건 ·
[기능명세 v1.7](../../기능명세/version1.7/버튼_갱신_파이프라인.md)(7단계) ·
🆕 [v1.8](../../기능명세/version1.8/기업행위_이벤트_표.md)(8단계 · §7) · PR #152
...
| ③ | 🆕 `action` 기업행위 표 | 종료코드 0 · **끄는 스위치 없음** | 264초 |
...
### 8.0 🆕 `action` 이 왜 달력 뒤이고 수정주가 앞인가 (v1.3)

`corporate_action`(스키마 v16 · 44,290건)도 `trading_calendar` 와 같은 **파생 표**라 시세가
늘면 곧바로 낡습니다. 자리를 여기로 잡은 이유가 둘입니다.

| | 왜 |
|---|---|
| 달력 **뒤** | `is_series_restart` 가 거래일 공백을 세는 데 달력이 든다. 달력이 낡으면 공백을 못 세고 **사건 종류가 바뀐다** |
| 수정주가 **앞** | 대조하는 chain 계수를 `adj_close` 가 아니라 `factor_series` 에서 **직접** 얻으므로 `adj` 에 안 기댄다. 평소에 꺼 두는 단계와 운명을 같이하지 않게 |

§8.1 이 달력을 `adj` 에서 떼어낸 것과 **정확히 같은 판단**입니다. 값이 싼 일(여기서는 4분)을
비싼 일(13분) 옆에 두면 같이 꺼집니다.

⚠️ 이 단계는 값을 **고치지 않습니다.** 증거와 chain 계수를 나란히 적고 어긋난 자리를 표시할
뿐이라, 붉은 자리가 나와도 여기서 멈추지 않습니다 — 멈춰야 하는지는 ⑤`gate`·⑥`verify` 가
정합니다.
...
## 9. 🆕 봉인 반출 — 개발본과 같은 함수로, 기록과 함께만 (v1.4 · 2026-09-11 · 이슈 #240)

> 정본: [`scripts/export_holdout_dataset.py`](../../../scripts/export_holdout_dataset.py) ·
> [`scripts/unseal_holdout.py`](../../../scripts/unseal_holdout.py) ·
> [`supply/holdout.py`](../../../supply/holdout.py) · 입구 해설은 [③ §9](03-공급-정문-asof.md) ·
> 절차는 [홀드아웃 개봉 절차](../../데이터파트/version4.5/홀드아웃_개봉_절차.md)

§1 의 봉인은 *"개발본에 홀드아웃이 한 줄도 없다"* 였습니다. 최종 평가 때는 그 반대 파일이 필요합니다 —
**홀드아웃이 든 파일**. 이 절은 그 파일을 만드는 길을 어떻게 좁혔는지입니다.

### 9.1 반출이 곧 개봉이다 — CLI 는 드라이런만 받는다

```python
# scripts/export_holdout_dataset.py
...
반출만 따로 돌릴 수 있으면 기록 없이 한 번 열립니다. 그래서 진짜 반출은 `unseal_holdout.py open` 이 사전
조건(사전등록 커밋 · 추적 파일 변경 0 · 데이터 게이트)을 확인한 뒤 `export_holdout()` 을 부르고 **곧바로**
기록을 쓰는 한 길뿐입니다.

### 9.2 같은 함수 — 스크립트가 둘이면 칸이 갈라진다

```python
# scripts/export_team_dataset.py
def build_full_daily(*, end, as_of, ctx):
    # 전 종목 시세 + 업종 · 주권종류 · 기업행위 판정 · 품질 플래그 — 개발본과 봉인본이 같이 쓴다
```

| 반출 | 부르는 곳 | `end` |
|---|---|---|
| 개발본 `full/daily_price_dev.parquet` | `export_team_dataset.main` | `DEV_END` (20240831) |
| 봉인본 `full/daily_price_all.parquet` | `export_holdout_dataset.export_holdout` | 달력 끝 (드라이런은 `DEV_END`) |

홀드아웃 성적이 개발구간과 **다른 코드로 만든 자료**로 매겨지면 비교가 무너집니다. 이번 판에서 하루
누수를 고친 붙이기([③ §8](03-공급-정문-asof.md))가 두 반출에 **동시에** 들어가는 것도 이 함수 하나 덕입니다.

🔴 **개발구간까지 함께 싣는 이유.** 피처는 롤링 창이라 홀드아웃 첫날에도 그 앞 행이 필요하고
(`hv_regime` 준비 구간 269행), 기업행위·품질 판정도 종목의 앞뒤 행을 봅니다. 잘라 읽으면 경계에서 답이
달라집니다.

### 9.3 범위는 DB 를 읽기 전에 막는다

```python
def validate_range(*, holdout_start, end, dry_run):
    if dry_run:
        if holdout_start >= HOLDOUT_START:
            raise ValueError("🔴 드라이런의 가짜 홀드아웃 시작은 진짜 봉인 시작보다 앞이어야 한다.")
        if end > DEV_END:
            raise ValueError("🔴 드라이런은 개발구간 끝까지만 읽는다")
    elif holdout_start != HOLDOUT_START:
        raise ValueError("진짜 반출의 홀드아웃 시작이 코드 정본과 다르다.")
```

드라이런은 개발구간 끝에서 **60거래일 전**을 가짜 시작으로 삼습니다(모델 파트 드라이런 준비기와 같은 길이).
읽은 뒤에는 §1 과 똑같이 `verify_no_holdout` 으로 **파일을 다시 열어** 진짜 봉인 구간이 없는지 전수로
봅니다 — 범위 검사를 믿지 않고 한 번 더.

### 9.4 덮어쓰지 않는다 — 지문이 곧 기록이다
...
기록 한 줄에는 `MANIFEST_holdout.json` 의 SHA-256 이 들어가고, 대장 안에는 파일 둘의 SHA-256 이 있습니다.
반출본을 다시 만들면 `generated_at` 하나만 달라도 지문이 바뀌어 기록과 안 맞습니다 — 그러면 **두 번째
개봉**입니다.

봉인본 대장에는 개발본 대장(`MANIFEST.json`)에 없던 칸이 넷 더 있습니다.

| 칸 | 왜 |
|---|---|
| `git_commit` | 무엇으로 반출했나 — 개발본 대장에는 아직 없다([명세서 §6](../../데이터파트/version4.5/명세서.md) 빚) |
| `db_schema_version` | 어느 스키마에서 |
| `holdout_segment` | 구간의 행 · 거래일 · 종목 · 결측 · 플래그 수 |
| `quality_ledger` | 어느 품질 판정 위에 섰나 (원장 이력의 머리) |
