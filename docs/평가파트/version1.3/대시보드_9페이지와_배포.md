...
> 실측 기준 main `c9cb120` (§1 Baseline 줄 · §4 상태 열은 main `694d9b3` · 11:16 KST 보강) · 대시보드 공개 URL https://alphastack-qurious.streamlit.app/ (Streamlit Cloud) ·
...
| `1_Baseline` | ① **ADAPTIVE** — 6-파라미터 동적 기준선 · 12폴드 · CMA-ES QUICK(30) / FULL(300) · 임계 1% / 2% (폴드마다 α/β 재최적화) ② 🆕 **FROZEN CLASSIFIER** — 전체 구간에서 CMA-ES 한 번 → 고정 α/β 한 세트 · 목적 조화평균(ACC·F1·하락 재현율) 또는 F1 · 연도별 안정성 (PR #269 · 이슈 #263 1차 구현) → ⛔ **제외 결정** (2026-09-14 11:10 KST · 강민석 · [#263](https://github.com/devlee328288/Alpha_Stack/issues/263)) · 코드는 main `694d9b3` 에 아직 있다 — 제거 PR 대기 | `evaluation/threshold_tuning/step5_optimize_6params.py` — `run_walkforward_6params` · 🆕 `find_frozen_6params` · `make_baseline_labels` |
...
| # | 문제 | 협업 파트 | 상태 (2026-09-14 11:16 KST · 댓글 시각 KST) |
|---|---|---|---|
| [#263](https://github.com/devlee328288/Alpha_Stack/issues/263) | Baseline 고정 분류기 부재 — 폴드마다 α/β 재최적화 | ML · 백테스팅 | ⛔ **제외 결정** — 강민석 11:10 · 코드 제거 PR 대기 (ML 의견 10:13) |
...
- 🔴 결정과 코드는 따로입니다 — main `694d9b3` 의 `dashboard/pages/1_Baseline.py` 에 Frozen 이 그대로 있고, HF Space 빌드(10:36 clone · main `c95ef6f` — #269 포함)에도 들어 있습니다. 제거 PR 이 머지되면 Streamlit Cloud 는 자동 반영, **Space 는 Factory rebuild** 가 필요합니다([#274](https://github.com/devlee328288/Alpha_Stack/issues/274)).
