...
def _fix_unadjusted_actions(rows: Sequence[Mapping],
    ...
    **① 원가격이 계수만큼 실제로 튀었나** (`CA_PRICE_TOLERANCE`)
    ...
    `factors[i]` 를 그대로 믿으면 안 된다. 재개일 계수는 `adjustment_factor` 가
    **상장주식수 배율**과 **KRX 기준가**를 맞대어 정하는데, 그 판단이 틀릴 수 있는
    사건이 있었다.

        009415 태영건설우 20200922  주식수 ×0.51 인데 가격은 32,500 → 기준가 16,250
                                    인적분할이라 가치도 같이 나갔다. 배율 1.96 은 틀리다
        009410 태영건설   20240722  주식수 ×24.9 인데 KRX 등락률 0.00% — 출자전환이다

    2026-09-07 부터 `adjustment_factor` 는 기준가가 배율을 따르지 않으면 기준가를 쓰므로
    위 둘은 각각 0.5 와 1 로 바로 나온다. 그래도 이 관문은 남긴다 — 계수와 원가격이
    서로를 검산하는 자리라서다. **원문 종가가 그 계수만큼 튀었는지**를 먼저 본다.
