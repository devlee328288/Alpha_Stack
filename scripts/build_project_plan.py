...
OUT_DOCX = Path("docs/계획서/version5.3/프로젝트계획서.docx")
...
MEMBERS = [
    "이동원(팀장) – 데이터 수집·정제·라벨에 맞는 전처리·크롤링 / 문서·일정 관리",
    "오준영 – AI 모델 개발 및 학습",
...
METHODS = [
    ...
    "상위 5개를 고른다. 그날까지 알려진 업종만 써서 미래정보를 막는다",
...
SCOPE_ROWS = [
    ...
    ["선택\n✅ 완료",
     ...
     "이동원·강민석",
     "2026-09-05 구현 (기능명세 v1.6 · 시험 25건)\n"
     "🔴 수정주가는 기본으로 끈다 — 매번 돌리면 FDR\n"
     "3,000거래일 창이 밀려 한 달에 11,502행이 원본에서\n"
     "우리 계산값(chain)으로 내려앉는다\n"
     "버튼 배선은 화면 파트"],
    ["2·3차\n계약",
     "⑮ 비LLM 에이전트 관측 규격\n(W×F 배열 · 명세만)",
     "이동원",
...
#: 착수 시점(8/25 이관) → 2026-09-05 실측. 줄 수는 `wc -l` · 조사용 `_probe*.py` 제외
...
ASSET_ROWS = [
    ...
    ["ingest/ — 수집·반입·적재", "7,488 → 14,958",
     "동작 중. 시세 9,231,938행 · 재무 662,933행 · 거시 17,851행\n"
     "수정주가 v9 · 종목 신원 v10 (코드↔법인등록번호↔ISIN)"],
    ["evaluation/ — 성과 검증", "772 → 5,778",
     "동작 중. 강민석 지표 2편 합류"],
    ["supply/ — 시점정합 공급", "228 → 1,319",
     "동작 중. 문 둘(예측·학습) · 홀드아웃 2년"],
    ["common/ — 설정·예산·robots·달력", "2,348 → 3,276",
     "동작 중. 거래일 달력 12,306행 · 반출 프로파일"],
    ["timeseries/ — ARIMA·ADF·ACF", "2,063 → 2,085",
     "코드는 있으나 아직 호출처가 없다"],
    ["scripts/ — 실행 CLI", "2,523 → 20,127",
     "동작 중 (수집·품질·반출·HF·키 점검·신원검증)"],
    ["tests/", "3,017 → 10,724", "924 passed · 3 xfailed · 259초"],
    ["features/ — 피처", "35 → 1,472", "지표 22칸 (신장환)"],
    ["models/ — 모델", "34 → 875", "4종 팩토리 (오준영)"],
    ["pipelines/ — 오케스트레이션", "0 → 901",
     "동작 중. ingest(수집) · refresh(버튼 갱신 · 6단계 · 잠금)"],
...
QFRS_ROWS = [
    ...
    ["QFRS-4\nScaling / Normalisation",
     ...
     "횡단면은 그날 안에서 끝나 미래를 못 본다.\n"
     "🔴 다만 전처리 후보 여덟을 12폴드로 재 보니 어느 것도 원값을\n"
     "넘지 못해 (최선 D +0.15%p·p=0.535, 최악 E 업종 중립화 −3.47%p)\n"
...
def appendix_blocks() -> List[Block]:
    ...
    b += [
        ...
        ("para", "필수 범위는 2주 안에 반드시 끝낸다. 선택 범위는 9/8 중간 "
                 ...
                 "⑭~⑯ 은 v3.0 에서 더한 데이터 파트 고도화다 — ⑭ 는 2026-09-05 에 "
                 "만들었고 ⑮⑯ 은 설계까지다.",
         9.5, False),
    ...
    b += [
        ...
        ("heading", "부록 D. 착수 자산"),
        ("para", "착수 시점(8/25 이관)의 자산과 2026-09-05 실측을 나란히 적는다. "
        ...
        ("para", "QFRS-4 를 채우는 과정에서 표준을 그대로 옮기면 안 된다는 것을 "
                 "실측으로 확인했다. 퀀트 표준(Barra·qlib·Alphalens)의 횡단면 "
                 "winsorize + z-score 를 걸었더니 기준선 대비 정확도가 1.58%p "
                 "떨어졌다. 우리 라벨이 절대 ±2% 밴드라 변동성 수준이 라벨의 "
                 "절반을 설명하는데(hv_20 십분위와 비중립 비율의 스피어만 상관 "
                 "1.0000) 날짜별 z-score 가 그 수준을 지우기 때문이다. 대안 "
                 "여덟을 12폴드 통제 실험으로 재고 전부 기각했다. 표준이 틀린 "
                 "것이 아니라 그 표준이 전제하는 상대 목표와 우리 라벨이 다르다.",
                 9.5, False),
        ("blank",),
        ("para", "여덟 중 마지막 셋(업종 중립화·시총 중립화·횡단면 순위)을 재면서 "
                 "손실의 구조도 갈랐다. 중립화는 회귀 설계행렬에 늘 절편이 들어가므로 "
                 "어느 축을 지우든 그날 평균이 먼저 지워진다 — '시총 중립화' 의 잔차가 "
                 "그냥 그날 평균을 뺀 값과 상관 0.96~0.98 이었고 시총이 추가로 설명한 "
                 "몫은 4.5%뿐이었다. 업종 라벨을 무작위로 섞어 자유도만 같게 만든 "
                 "대조군을 세워 다시 가르면, 업종 중립화의 손실 3.47%p 는 날짜 수준 "
                 "48% · 자유도 33% · 업종 정보 19% 로 나뉜다. 연산의 이름이 그 연산이 "
                 "하는 일의 절반만 말할 수 있다는 뜻이고, 그래서 조건을 등록하기 전에 "
                 "무엇이 지워지는지를 값으로 먼저 재는 절차를 규칙으로 넣었다.",
                 9.5, False),
        ("blank",),
        ("para", "같은 실험에서 선정 축이 결론을 뒤집는 사례도 나왔다. 업종 중립화는 "
                 "정보계수의 안정성(ICIR)을 9.5% 올리지만 우리 판정 축인 정확도는 "
                 "3.47%p 낮춘다. 무작위 대조군에서는 ICIR 이 오르지 않으므로 그 상승은 "
                 "실제 업종 정보를 지운 결과다. 한 정보가 한 축에서는 잡음이고 다른 "
                 "축에서는 신호이므로, 결론에는 어느 축으로 판정했는지를 함께 적는다.",
                 9.5, False),
        ("blank",),
