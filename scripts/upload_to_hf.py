...
COLUMN_NOTES: Dict[str, str] = {
    ...
    "industry": "✅ KRX 업종명 (`전기·전자`·`IT 서비스`…). 사람이 KRX 화면에서 받은 "
                "연 1회 업종분류 현황 스냅샷을 **그 행의 날짜까지 알게 된 가장 최근 것**"
                "으로 붙였다. 스냅샷 당일 행에는 그날 표가 아니라 **그 앞 스냅샷**이 붙는다"
                "(2026-09-11 고침 — 그날 표는 마감 뒤에 나온다). "
                "그해 스냅샷 뒤에 상장한 종목은 다음 스냅샷까지 빈다. "
    ...
    # ── 주권종류 — KRX 종목기본정보, 그 행의 날짜까지 알게 된 값(보통 직전 거래일) ──
    # 🔴 2026-09-11 고침: 예전에는 같은 날짜로 붙여 하루 앞선 값을 실었다. 상장 첫날은 빈다.
    "kind_stkcert_tp_nm": "✅ **주권종류** — `보통주`·`구형우선주`·`신형우선주`·`종류주권`. "
                          "KRX 가 정한 값이라 **종목명으로 추측하지 않는다.** "
                          "이름이 '우' 로 끝나는 보통주(연우·동우·신우·미래에셋대우)를 "
                          "이름 규칙은 우선주로 잘못 뺐다. 보통주만 쓰려면 "
                          "`df[df.kind_stkcert_tp_nm == '보통주']`",
    "secugrp_nm": "증권그룹 — `주권`·`외국주권`·`부동산투자회사`(리츠)·`선박투자회사`·"
                  "`투자회사`·`사회간접자본투융자회사`·`주식예탁증권`·`주식예탁증서`. "
                  "CRSP 표준은 REIT·closed-end fund·외국주권·ADR 을 유니버스에서 뺀다",
    "sect_tp_nm": "소속부 — `우량기업부`·`중견기업부`·`벤처기업부`·`기술성장기업부`·"
                  "`SPAC(소속부없음)`·`관리종목(소속부없음)`·`투자주의환기종목(소속부없음)`·"
                  "`외국기업(소속부없음)`. KOSPI 는 대체로 빈 값이다. "
                  "KOSPI200 지수 방법론은 SPAC·관리종목을 후보에서 뺀다",
    # ── 기업행위 판정 — 🔴 표본 선택용, 피처 금지 ──
    "is_liquidation": "🔴 **정리매매** — 체결이 끊기기 직전 10체결일. "
                      "**표본에서 빼는 용도로만** 쓴다. \"이 뒤로 체결이 끊긴다\" 를 보고 "
                      "매기므로 **피처로 넣으면 미래참조다**",
    "is_halted": "🔴 **거래정지** — 그 행에 체결이 없었다(`open<=0` 또는 `volume<=0`). "
                 "KRX 는 정지 중에도 행을 주는데 시·고·저가가 0 이고 종가만 직전 값을 "
                 "물고 있다. **표본에서 빼는 용도로만** 쓴다",
    "is_first_listing": "🔴 **신규상장 첫 거래일** — 등락률이 전일종가가 아니라 **공모가** "
                        "기준이라 수익률이 그 날만 다른 뜻이다. 수집 시작일(2010-01-04)의 "
                        "첫 행은 상장이 아니라 수집 경계라 참이 아니다. "
                        "**표본에서 빼는 용도로만** 쓴다",
    "index_name": "지수 이름",
    "index_class": "지수 구분",
    # ── 원문 가격 ──
    ...
    "adj_close": "✅ **수정 종가** — 수익률·라벨·모멘텀은 전부 이 칸으로. "
                 "다만 **현금배당은 안 들어 있다** (CRSP 로 치면 `RETX`). "
                 "배당까지 담은 값은 `adj_close_tr`",
    "adj_source": "그 행의 수정값이 어디서 왔나. "
                  "`fdr` 은 외부 실측, `chain` 은 우리가 이어 붙인 값",
    # ── 총수익 ── 🔴 성과 평가용, 피처 금지
    "adj_close_tr": "✅ **총수익 지수** — 배당까지 재투자한 값 (CRSP 의 `RET` 쪽). "
                    "첫 거래일에 `adj_close` 와 같은 값에서 출발해 "
                    "`(adj_close + adj_dividend) / 직전 adj_close` 를 누적한다. "
                    "🔴 **성과 축이지 피처가 아니다** — 배당 확정은 주주총회라 그 전에는 "
                    "금액을 알 수 없어서, 입력으로 넣으면 미래참조가 된다. "
                    "성과·ΔSharpe·누적수익 비교에만 쓴다. "
                    "가격만 볼 때는 `adj_close` 를 그대로 쓰면 된다",
    "adj_dividend": "그날 더해진 주당 배당금 — **원금액이 아니라 수정 배율을 태운 값**이다 "
                    "(`배당금 × adj_close/close`). 배당락일에만 값이 있고 나머지 날은 비어 "
                    "있다. 배당구분이 `현금배당`·`동시배당` 인 것만 담았다 — 주식배당은 "
                    "수정주가가 이미 펴서 또 더하면 두 번 센다. "
                    "🔴 `adj_close_tr` 과 같은 이유로 **피처로 쓰면 미래참조다**",
    "is_dividend_suspect": "🔴 원본 배당금의 **단위 오류**. 참인 행은 `adj_dividend` 를 "
                           "비우고 총수익에서 뺐다 (전 구간 1행 — 윙입푸드홀딩스 20181227, "
                           "주당 18.5억). **크기로 자르지 않았다** — 배당이 종가보다 큰 행 "
                           "넷 중 셋은 유전펀드·리츠의 정당한 청산분배금이라 함께 자르면 "
                           "총수익이 아래로 편향된다. 대신 금액과 배당률로 그날의 액면가를 "
                           "되짚어 서로 맞는지 봤다 (조인 대상의 99.29%가 맞는다)",
    # ── 피처 ──
...
FILE_HEADLINE: Dict[str, str] = {
    "full/daily_price_dev.parquet":
        "개발구간 전 종목 시세 — **판정을 안 걸러 놓은 원본**. "
        "정리매매·거래정지·신규상장이 그대로 있고, 어느 행인지는 `is_liquidation`·"
        "`is_halted`·`is_first_listing` 로 표시돼 있다 (아래 §표본 참조)",
