...
def print_gamma_results(results: Dict[float, Dict]):
    ...
    # 테이블 헤더
    print(
        f"{'γ':>6} | {'Mean':>8} | {'Std':>8} | {'Median':>8} | {'Min':>8} | {'Max':>8} | {'Folds':>5}"
    )
    print("-" * 70)
    ...
    for gamma in sorted(results.keys()):
        ...
        if r["n_folds"] == 0:
            print(
                f"{gamma:>6.1f} | {'N/A':>8} | {'N/A':>8} | {'N/A':>8} | {'N/A':>8} | {'N/A':>8} | {0:>5}"
            )
        else:
            print(
                f"{gamma:>6.1f} | {r['mean']:>8.4f} | {r['std']:>8.4f} | {r['median']:>8.4f} | {r['min']:>8.4f} | {r['max']:>8.4f} | {r['n_folds']:>5}"
            )
            if r["mean"] > best_mean:
                best_mean = r["mean"]
                best_gamma = gamma
