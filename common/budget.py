...
#   data_go_kr      10,000  ⚠️ 공공데이터포털 **개발계정의 API 별 일일 트래픽 기본값**이다.
#                           포털은 활용신청한 API 마다 따로 세는데 우리는 금융위(1160100)
#                           와 예탁결제원(B552481) 호출을 **한 통으로** 센다 — 그래서 이
#                           값은 보수적이다. 넘길 위험은 없고 덜 쓸 뿐이다. 실제 소진은
#                           `LIMITED_NUMBER_OF_SERVICE_REQUESTS_EXCEEDS_ERROR` 로 서버가
#                           알려 주고 그때 `mark_exhausted()` 가 받는다.
#                           2026-09-03 까지는 이 표에 없어 호출부가 `limit=` 을 매번
#                           넘겼다. 두 클라이언트가 같은 통을 쓰려면 정본이 하나여야 한다.
#
